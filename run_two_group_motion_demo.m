...
%
% Useful modes:
%   run_two_group_motion_demo('Repetitions', 5)
%   run_two_group_motion_demo('UseTrialSubset', true, 'SubsetTrialCount', 16)
%   run_two_group_motion_demo('SmokeTest', true, 'SkipEyeLink', true)
%   run_two_group_motion_demo('InteractiveSmokeTest', true)
%   r = run_two_group_motion_demo('ValidateOnly', true)
%   r = run_two_group_motion_demo('DataSelfTest', true)
%   run_two_group_motion_demo('GenerateFigures', true)
%   run_two_group_motion_demo('SoundTest', true)
%  run_two_group_motion_demo('SkipEyeLink', true)
...
[schedule, schedule_coherence_indices, schedule_rotation_indices, ...
    ...
    numel(cfg.condition_inner_direction_deg), ...
    numel(cfg.coherence_values_percent), ...
    numel(cfg.rotation_values_deg), ...
    cfg.repetitions_per_condition, ...
    cfg.number_of_blocks, ...
    cfg.use_trial_subset, ...
    cfg.subset_trial_count);
...
validation = validate_scientific_state(cfg, schedule, ...
    schedule_coherence_indices, ...
    schedule_rotation_indices, ...
    schedule_block_numbers, ...
    vectors);
...
try
        ...
        '%d directions x %d coherences x %d rotations x %d repetitions x %d blocks = %d trials\n\n' ...
        'Coherence values: %s %%\n' ...
        'Rotation values: %s degrees\n\n' ...
        ...
        mat2str(cfg.rotation_values_deg));
...
cfg.repetitions_per_condition = double(cfg.repetitions_per_condition);
cfg.number_of_blocks = double(cfg.number_of_blocks);
...
if ~isscalar(cfg.repetitions_per_condition) || ...
        ~isfinite(cfg.repetitions_per_condition) || ...
        cfg.repetitions_per_condition < 1 || ...
        cfg.repetitions_per_condition ~= ...
        round(cfg.repetitions_per_condition)

    error('repetitions_per_condition must be a positive integer.');
end

if ~isscalar(cfg.number_of_blocks) || ...
        ~isfinite(cfg.number_of_blocks) || ...
        cfg.number_of_blocks < 1 || ...
        cfg.number_of_blocks ~= round(cfg.number_of_blocks)

    error('number_of_blocks must be a positive integer.');
end
...
if cfg.use_trial_subset
    ...
    unique_conditions_per_block = ...
    n_directions * n_coherences * n_rotations;

    trials_per_block = ...
    unique_conditions_per_block * cfg.repetitions_per_condition;

    full_count = ...
    trials_per_block * cfg.number_of_blocks;

    if mod(cfg.subset_trial_count, trials_per_block) ~= 0
        error('two_group_motion_demo:InvalidSubsetTrialCount', ...
            ['subset_trial_count must be a multiple of the full block size ' ...
             '(%d = %d directions x %d coherences x %d rotations).'], ...
            trials_per_block, n_directions, n_coherences, n_rotations);
    end
...
function [condition_schedule, coherence_schedule, rotation_schedule, ...
        ...
        n_conditions, n_coherences, n_rotations, ...
        repetitions_per_condition, number_of_blocks, ...
        use_subset, subset_trial_count)

% ROTATION_BLOCK_SCHEDULE
%
% Each block contains the COMPLETE factorial design:
%   all directions x all coherences x all rotations
%
% Each unique combination is repeated:
%   repetitions_per_condition
%
% Example:
%   6 directions x 3 coherences x 6 rotations
%   = 108 unique conditions per block
%
%   With 2 repetitions:
%   108 x 2 = 216 trials per block
%
%   With 14 blocks:
%   216 x 14 = 3024 total trials

% ---------------------------------------------------------
% Calculate block and full-design sizes
% ---------------------------------------------------------
...
trials_per_block = ...
    unique_conditions_per_block * repetitions_per_condition;

full_design_trial_count = ...
    trials_per_block * number_of_blocks;


% ---------------------------------------------------------
% Determine how many trials will actually be scheduled
% ---------------------------------------------------------
...
if mod(scheduled_trial_count, trials_per_block) ~= 0
    error('two_group_motion_demo:InvalidSubsetTrialCount', ...
        ['subset_trial_count must be a multiple of the full block size ' ...
         '(%d trials = %d directions x %d coherences x %d rotations ' ...
         'x %d repetitions).'], ...
         trials_per_block, ...
         n_conditions, ...
         n_coherences, ...
...
% ---------------------------------------------------------
% Determine number of blocks actually scheduled
% ---------------------------------------------------------

scheduled_block_count = ...
    scheduled_trial_count / trials_per_block;


% ---------------------------------------------------------
% Make sure subset does not exceed the configured design
% ---------------------------------------------------------

if scheduled_trial_count > full_design_trial_count
    error('two_group_motion_demo:SubsetExceedsFullDesign', ...
        ['scheduled_trial_count (%d) exceeds the full design ' ...
         '(%d trials).'], ...
...
% ---------------------------------------------------------
% Create the complete factorial condition set
%
% Every cell represents:
%   direction x coherence x rotation
% ---------------------------------------------------------

[cell_condition, cell_coherence, cell_rotation] = ...
    ndgrid( ...
        1:n_conditions, ...
        1:n_coherences, ...
        1:n_rotations);
...
for block_index = 1:scheduled_block_count
    ...
    % Repeat every unique condition the requested number of times.
    %
    % Example:
    %   108 unique conditions
    %   x 2 repetitions
    %   = 216 trials
    block_condition = repmat( ...
        cell_condition, ...
        1, ...
        repetitions_per_condition);

    block_coherence = repmat( ...
        cell_coherence, ...
        1, ...
        repetitions_per_condition);

    block_rotation = repmat( ...
        cell_rotation, ...
        1, ...
        repetitions_per_condition);


    % Randomize all trials within this block.
    %
    % This preserves the required number of occurrences of every
    % direction x coherence x rotation combination.
    trial_order = randperm(numel(block_condition));


    % Indices corresponding to this block in the full schedule.
    indices = ...
        (block_index - 1) * trials_per_block + ...
        (1:trials_per_block);


    % Store randomized block.
    condition_schedule(indices) = ...
        block_condition(trial_order);

    coherence_schedule(indices) = ...
        block_coherence(trial_order);

    rotation_schedule(indices) = ...
        block_rotation(trial_order);

    block_schedule(indices) = ...
        block_index;
...
meta = struct( ...
    'full_design_trial_count', full_design_trial_count, ...
    'scheduled_trial_count', scheduled_trial_count, ...
    'full_block_count', number_of_blocks, ...
    'scheduled_block_count', scheduled_block_count, ...
    'trials_per_block', trials_per_block, ...
    'unique_conditions_per_block', unique_conditions_per_block, ...
...
function validation = validate_scientific_state(cfg, schedule, ...
        coherence_schedule, rotation_schedule, block_schedule, vectors)
...
if isempty(block_schedule)
    errors{end + 1} = 'block schedule is empty';
    validation = struct( ...
        'passed', false, ...
        'errors', {errors});
    return;
...
for block_index = 1:n_blocks
    ...
    if sum(rows) ~= expected_trials_per_block
        errors{end + 1} = sprintf( ...
            'block %d has %d trials; expected %d', ...
            block_index, ...
            sum(rows), ...
            expected_trials_per_block);
    end
    ...
    % Check every direction x coherence x rotation
    % combination occurs the requested number of times.

    for i = 1:n_conditions

        for j = 1:n_coherences
            ...
            for k = 1:n_rotations
                ...
                count = sum( ...
                    rows & ...
                    schedule == i & ...
                    coherence_schedule == j & ...
                    rotation_schedule == k);

                if count ~= cfg.repetitions_per_condition

                    errors{end + 1} = sprintf( ...
                        ['block %d, direction %d, coherence %d, rotation %d ' ...
                         'has %d trials; expected %d'], ...
                        block_index, ...
                        i, ...
                        j, ...
                        k, ...
...
% ---------------------------------------------------------
% Check total counts when the full design is being used
% ---------------------------------------------------------

if ~cfg.use_trial_subset

    expected_count = ...
        cfg.repetitions_per_condition * cfg.number_of_blocks;
        ...
        for j = 1:n_coherences
            ...
            for k = 1:n_rotations
                ...
                if actual_count ~= expected_count
                    ...
                    errors{end + 1} = sprintf( ...
                        ['condition %d, coherence %d, rotation %d ' ...
                         'has %d trials; expected %d'], ...
                        i, ...
                        j, ...
                        k, ...
...
validation = struct( ...
    ...
    'trials_per_block', expected_trials_per_block, ...
    'repetitions_per_condition', cfg.repetitions_per_condition, ...
    'trials_per_condition', ...
        n_blocks * cfg.repetitions_per_condition, ...
...
row.rotation_block_number = ...
    ceil(trial_index / trials_per_block);

row.rotation_block_position = ...
    mod(trial_index - 1, trials_per_block) + 1;
...
row.repetitions_per_condition = ...
    cfg.repetitions_per_condition;
row.number_of_blocks = cfg.number_of_blocks;
