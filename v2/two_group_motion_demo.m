...
try
    instruction_text = sprintf([ ...
        ...
        'Full design: '%d directions x %d coherences x %d rotations x %d repetitions x %d blocks = %d trials\n' ...
...
%
% Example:
%   6 directions x 6 coherences x 3 rotations = 108 trials/block
...
if mod(scheduled_trial_count, trials_per_block) ~= 0
    error('two_group_motion_demo:InvalidSubsetTrialCount', ...
        ['subset_trial_count must be a multiple of the full block size ' ...
         ...
         trials_per_block, n_conditions, n_coherences, n_rotations);
end
...
[cell_condition, cell_coherence, cell_rotation] = ...
    ndgrid(1:n_conditions, 1:n_coherences, 1:n_rotations);
...
for block_index = 1:scheduled_block_count
    ...
    block_condition = repmat( ...
        cell_condition, 1, repetitions_per_condition);

    block_coherence = repmat( ...
        cell_coherence, 1, repetitions_per_condition);

    block_rotation = repmat( ...
        cell_rotation, 1, repetitions_per_condition);
...
for block_index = 1:n_blocks
   ...
   expected_block_trials = ...
    n_conditions * n_coherences * n_rotations * ...
    cfg.repetitions_per_condition;

if sum(rows) ~= expected_block_trials
    errors{end + 1} = sprintf( ...
        'block %d has %d trials; expected %d', ...
        block_index, sum(rows), expected_block_trials);
end
        errors{end + 1} = sprintf( ...
    ...
    for i = 1:n_conditions
        for j = 1:n_coherences
            for k = 1:n_rotations
                ...
                actual_count = sum( ...
                    schedule == i & ...
                    schedule_coherence_indices == j & ...
                    schedule_rotation_indices == k);

                if actual_count ~= expected_count
                    errors{end + 1} = sprintf( ...
                        ['condition %d, coherence %d, rotation %d ' ...
                         'has %d trials; expected %d'], ...
                        i, j, k, actual_count, expected_count);
                end
            end
        end
    end
